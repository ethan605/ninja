...
struct NodeStoringImplicitDepLoader : public ImplicitDepLoader {
  NodeStoringImplicitDepLoader(State* state, DepsLog* deps_log, DiskInterface* disk_interface,
                               DepfileParserOptions const* depfile_parser_options, std::vector<Node*>* dep_nodes_output)
...
bool NodeStoringImplicitDepLoader::ProcessDepfileDeps(Edge* edge, std::vector<StringPiece>* depfile_ins,
                                                      std::string* err) {
...
void MissingDependencyPrinter::OnMissingDep(Node* node, const std::string& path, const Rule& generator) {
  std::cout << "Missing dep: " << node->path() << " uses " << path << " (generated by " << generator.name() << ")\n";
}

MissingDependencyScanner::MissingDependencyScanner(MissingDependencyScannerDelegate* delegate, DepsLog* deps_log,
                                                   State* state, DiskInterface* disk_interface)
    : delegate_(delegate), deps_log_(deps_log), state_(state), disk_interface_(disk_interface),
      missing_dep_path_count_(0) {}
...
void MissingDependencyScanner::ProcessNodeDeps(Node* node, Node** dep_nodes, int dep_nodes_count) {
  Edge* edge = node->in_edge();
  ...
  for (std::set<Edge*>::iterator de = deplog_edges.begin(); de != deplog_edges.end(); ++de) {
  ...
  if (!missing_deps.empty()) {
    ...
    for (std::vector<Edge*>::iterator ne = missing_deps.begin(); ne != missing_deps.end(); ++ne) {
      for (int i = 0; i < dep_nodes_count; ++i) {
...
void MissingDependencyScanner::PrintStats() {
  ...
  if (HadMissingDeps()) {
    std::cout << "Error: There are " << missing_dep_path_count_ << " missing dependency paths.\n";
    std::cout << nodes_missing_deps_.size() << " targets had depfile dependencies on " << generated_nodes_.size()
              << " distinct generated inputs "
